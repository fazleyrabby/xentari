...
export function detectEntryPoints(files) {
    return files
        .filter(f => f.file.includes("app.js") || f.file.includes("app.ts") ||
...
function detectModules(files) {
    ...
    files.forEach(f => {
        ...
        if (lower.includes("auth") || lower.includes("login"))
            ...
            mod = "todos";
        else if (lower.includes("pay") || lower.includes("billing"))
            mod = "payments";
        ...
        const sorted = [...files].sort((a, b) => {
            const getRank = (p) => {
                ...
                if (p.includes("service"))
                    return 3;
                if (p.includes("model") || p.includes("schema"))
                    return 4;
                return 5;
            };
...
export async function indexProject(projectDir) {
    ...
    Object.entries(knowledge.dependencies).forEach(([file, deps]) => {
        deps.forEach(dep => {
            var _a;
            (_a = knowledge.reverseDependencies)[dep] ?? (_a[dep] = []);
    ...
    if (!existsSync(xentariDir))
        mkdirSync(xentariDir, { recursive: true });
