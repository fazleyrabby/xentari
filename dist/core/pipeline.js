...
import { statusBar } from "./tui/index.js";
...
async function codeAndReview(step, files, maxAttempts, chainContext, projectDir) {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
        if (attempt > 1)
            log.info(`Retry ${attempt}/${maxAttempts}`);
...
async function processStep(step, index, opts, chain) {
    ...
    try {
        ...
        files = [{
                file: (step.filePath || (step.files && step.files[0]) || ""),
...
export async function runPlanOnly({ task, projectDir }) {
    log.section("PLAN");
